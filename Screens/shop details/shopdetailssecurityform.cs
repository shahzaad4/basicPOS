...
{
    public partial class shopdetailssecurityform : MetroFramework.Forms.MetroForm
    {
        public shopdetailssecurityform()
        {
            InitializeComponent();
        }

        private void shopdetailssecurityform_Load(object sender, EventArgs e)
        {
            ...
            if (isValid())
            {
                ...
                using (SqlConnection con = new SqlConnection(ApplicationSetting.ConnectionStrin()))
                {
                    using (SqlCommand cmd = new SqlCommand("usp_Login_VerifyLoginDetails", con))
                    {
                        ...
                        {
                            this.Hide();
                            Editshopdetailsform  df = new Editshopdetailsform();
                            df.Show();
                        }
                        else
                        {
